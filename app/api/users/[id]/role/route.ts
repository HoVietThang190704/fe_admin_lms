...
const buildRoleEndpoint = (userId: string) => {
  const template = INTERNAL_API_ENDPOINTS.USERS.ROLE || '/api/users/{id}/role';
...
export const PATCH = apiHandlerWithReq(async (req) => {
  ...
  const { data } = await backendFetch(buildRoleEndpoint(userId), {
    method: 'PATCH',
    headers,
